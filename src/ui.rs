...
pub fn start(args: &mut [String]) {
    ...
    #[cfg(windows)]
    {
        ...
        // If the DLL is not next to the executable, extract the embedded one to %TEMP%
        if !loaded {
            let bytes = include_bytes!("../sciter.dll");
            let temp_dir = std::env::temp_dir();
            let dll_path = temp_dir.join("rustdesk_sciter.dll");
            std::fs::write(&dll_path, bytes).ok();
            sciter::set_library(&dll_path.to_string_lossy().to_string()).ok();
        }
    }
